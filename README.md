...
手元の `.asc` を 9 個調べました（LTspice 17.0.37.0 が書いたものと、新しい版が
書いたもの、`Version 4` と `4.1` の両方）。

- **どれも単バイト**でした。UTF-16 のものは1つもありません
- 改行はどれも CRLF
- 非 ASCII は素子値の µ だけで、**0xB5 の1バイト**（Windows-1252 / Latin-1）。
  UTF-8 の µ は2バイトなので、UTF-8 として読むと化けます
- **µ が入るかどうかは版と関係がありませんでした。** 同じ回路・同じ版で、
  `100u` と書かれたものと `100µ` と書かれたものの両方があります

さらに、**同じ µ でもファイルの種類でバイトが違いました**。

| | µ のバイト | |
|---|---|---|
| `.asc` | `b5` | 1 バイト（Windows-1252 / Latin-1） |
| `.net` | `c2 b5` | 2 バイト（UTF-8） |

同じ LTspice が同じ回路から書いたものでも、こうなります。`pkg/netlist` が
UTF-8 の扱いで読めていたのに `pkg/asc` で化けたのは、これが理由です。

そこで、UTF-8 として通ればそのまま、通らなければ Windows-1252 とみなします。
UTF-16 の見分けも入れてありますが、これは `.raw` の読み取り（そちらは本当に
UTF-16LE のものがあります）と同じ仕組みを使い回しているだけで、**UTF-16 の
`.asc` は見たことがありません**。
