...
// Parse は開いてあるものを読む。
//
// 文字コードは3通りある。LTspice は .asc を**システムのコードページ**で
// 書くこともあれば UTF-16LE で書くこともある（版と設定による）。先頭の
// 2 バイトで UTF-16 を見分け、そうでなければ UTF-8 として読み、UTF-8 と
// して通らなければ Windows-1252 とみなす。素子値の µ が 0xB5 の1バイトで
// 入っているのが実際の例。
...
// --- 文字コード ---------------------------------------------------------
//
// .raw と同じで、版によって ASCII だったり UTF-16LE だったりする。
...
//
// LTspice は .asc を**システムのコードページ**で書く。日本語 Windows でも
// 素子値の µ は 0xB5 の1バイトで入っていて、UTF-8 として読むと壊れる。
// UTF-8 として通らなければ Windows-1252 とみなして読み直す。
//
